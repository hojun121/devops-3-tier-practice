...
> ⚠️ EC2가 프라이빗 서브넷에 위치하므로 NAT Gateway 없이는 외부 접속 불가. 패키지 설치, npm install, CodeDeploy 통신 모두 NAT 경유. NAT가 미연결 상태면 모든 단계에서 실패한다.
...
> ⚠️ `package-lock.json`이 없으면 배포 시 `after_install.sh`의 `npm ci`가 다음 오류로 실패한다. `The npm ci command can only install with an existing package-lock.json`. 본 작업 시작 전 로컬에서 `npm install` 1회 실행하여 lock 파일을 생성한 뒤 커밋한다.

### 3) 사전 확인 정보

진행 중 사용할 값을 사전에 메모한다.
...
| 프라이빗 서브넷 ID (svc) 2개 | VPC > Subnets |
| 프라이빗 서브넷 ID (db) 2개 | VPC > Subnets |
...
> ⚠️ 본 버킷 이름은 4-4-1의 Policy에 작성한 이름과 정확히 일치해야 한다.
...
### 5-4. CodeDeploy Application 및 Deployment Group 생성
...
### 7-3. RDS 스키마 적용 (1회 수행)

첫 배포 완료 후, RDS에 초기 스키마를 적용한다.
